...
function buildFileChip(
  ...
  if (handlers.onReferenceLocate) {
    ...
    locateBtn.title = t('fileEditor.locateFile')
  ...
  if (handlers.onReferenceDelete) {
    ...
    deleteBtn.className =
      'composer-file-ref-action inline-flex size-4 items-center justify-center rounded-sm'
    deleteBtn.title = t('fileEditor.deleteReference')
...
function renderDocument(
  ...
  const { highlightedFileId, t, ...handlers } = props
  ...
  for (const node of documentNodes) {
    ...
    root.append(
      buildFileChip(node, file, handlers, t, highlightedFileId),
