...
export function useMermaidThemeVersion(): number {
  // Theme is always dark — version never changes at runtime
  return 0
}
...
function buildThemeVars(): {
  ...
  const card = normalizeColor(
    readCssVar('--card', '#0e0e0e'),
    'rgb(14, 14, 14)'
  )
  const muted = normalizeColor(
    readCssVar('--muted', '#262626'),
    'rgb(38, 38, 38)'
  )
  const mutedForeground = normalizeColor(
    readCssVar('--muted-foreground', '#a3a3a3'),
    'rgb(163, 163, 163)'
  )
  const border = normalizeColor(
    readCssVar('--border', '#262626'),
    'rgb(38, 38, 38)'
  )
  return {
    dark,
...
export async function copyMermaidToClipboard(
  ...
  try {
    ...
    // Use the native clipboard command for reliable image writing
    const result = (await tauriCommands.invoke(TAURI_COMMANDS.CLIPBOARD_WRITE_IMAGE, {
      data: base64
    })) as {
      success?: boolean
      error?: string
    ...
    log.warn('TAURI_COMMANDS clipboard write failed:', result.error)
  } catch (err) {
    log.warn('PNG clipboard write failed:', err)
  }
