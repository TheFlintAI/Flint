...
interface StreamingPerformanceSnapshot {
  enabled: boolean
...
interface StreamingPerformanceApi {
...
function buildFailures(summary: StreamingPerformanceSnapshot['summary']): string[] {
...
export function isStreamingPerformanceEnabled(): boolean {
  return enabled
}
...
function snapshot(): StreamingPerformanceSnapshot {
  const summary = {
...
export function installStreamingPerformanceMonitor(): void {
  if (!enabled || installed) return
  installed = true

  window.__flintStreamingPerformance = {
    snapshot,
    reset
  }
...
declare global {
  interface Window {
    __flintStreamingPerformance?: StreamingPerformanceApi
  }
}
