...
/**
 * Map Flint's reasoning effort levels to native Gemini API thinking config.
 *
 * Gemini 3.x+ (3, 3.1, 3.5):  uses `thinkingLevel`  (string: low | medium | high)
 * Gemini 2.5:                  uses `thinkingBudget` (number: mapped from effort)
 *
 * Placed under `generationConfig.thinkingConfig` in the request body.
 */
function buildGeminiThinkingConfig(
  ...
  // Gemini 2.5 models use numeric thinkingBudget
  const isGemini25 = /gemini-2\.5/i.test(config.model)
  ...
  if (isGemini25) {
    ...
    return {
      thinkingBudget: budgetMap[config.reasoningEffort] ?? 4096,
      includeThoughts: true
    }
  }

  // Gemini 3.x+: use string thinkingLevel
  return {
  ...
  async *sendMessage(
    ...
    try {
      ...
      if (config.thinkingEnabled && config.thinkingConfig) {
        ...
        // Map Flint reasoning effort to native Gemini thinking config
        const geminiThinkingConfig = buildGeminiThinkingConfig(config)
    ...
    return [
      {
        functionDeclarations: tools.map((tool) => {
          if (typeof tool.description !== 'string') {
            throw new Error(
              `[Gemini] Tool "${tool.name}" has non-string description ` +
              `(type=${typeof tool.description}). This must be resolved before reaching formatTools.`
            )
          }
          return {
            name: tool.name,
