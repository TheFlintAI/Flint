...
import { streamApiRequestViaTauri, maskHeaders } from '@/services/tauri-api/api-stream'
import { registerProvider } from './provider'
import { normalizeMessagesForAnthropicToolReplay } from '@/protocols/anthropic-tool-replay'
...
class AnthropicProvider implements APIProvider {
  ...
  async *sendMessage(
    ...
    const formattedTools =
      tools.length > 0 ? this.formatTools(tools, promptCacheEnabled, cacheBudget, config.supportsStrictSchemas) : undefined
  ...
  formatTools(
    ...
    return tools.map((t, index) => {
      if (typeof t.description !== 'string') {
        throw new Error(
          `[AnthropicProvider] Tool "${t.name}" at index ${index} has non-string description ` +
      ...
      return {
        ...
        input_schema: this.normalizeToolSchema(t.inputSchema, supportsStrictSchemas),
  ...
  private normalizeToolSchema(
    ...
    if ('properties' in schema) {
      // For simple object schemas: keep as-is when strict schemas supported
      if (supportsStrictSchemas) return schema as Record<string, unknown>
      // Strip additionalProperties: false for providers that don't support strict schemas
      return this.sanitizeSchemaForCompat(schema as Record<string, unknown>)
    }
        ...
        if (!(key in mergedProperties)) {
          mergedProperties[key] = supportsStrictSchemas
            ...
            : value && typeof value === 'object' && !Array.isArray(value)
              ? this.sanitizeSchemaForCompat(value as Record<string, unknown>)
              : value
        }
      }
  ...
  /**
   * Strip `additionalProperties: false` recursively from a JSON Schema tree.
   * Preserves `additionalProperties` objects that define valid property shapes.
   */
  private sanitizeSchemaForCompat(obj: Record<string, unknown>): Record<string, unknown> {
        ...
        for (const [propKey, propVal] of Object.entries(value as Record<string, unknown>)) {
          cleaned[propKey] =
            propVal && typeof propVal === 'object' && !Array.isArray(propVal)
              ? this.sanitizeSchemaForCompat(propVal as Record<string, unknown>)
              : propVal
        }
        result[key] = cleaned
      } else {
